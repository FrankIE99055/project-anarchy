-- 05_planning_staging_and_bulk_load.sql
-- ------------------------------------------------------------------
-- Import stavebních povolení (National Planning Applications) přímo
-- do databáze - stejný princip jako u PPR (staging tabulka + hromadný
-- SQL insert, žádné API volání po řádku).
--
-- POSTUP:
-- 1) Spusť KROK 1 (staging tabulka).
-- 2) V Table Editoru nahraj data_pipeline/raw_data/planning_clean.csv
--    do tabulky `planning_staging` (Insert -> Import data from CSV).
-- 3) Spusť KROK 2 (doplnění properties o nové adresy z povolení) a
--    KROK 3 (naplnění planning_permissions).
-- ------------------------------------------------------------------
...
-- ============ KROK 2: doplnění properties o adresy z povolení ============
-- Adresy v tomto datasetu se textově neshodují přesně s PPR adresami,
-- takže většina skončí jako NOVÉ property záznamy (bez historie prodeje).
-- To je v pořádku - obohacuje to naši databázi o nemovitosti, které se
-- třeba ještě neprodaly, ale mají aktivní stavební povolení.
--
-- ITMEasting/ITMNorthing = souřadnice staveniště v irském souřadnicovém
-- systému (EPSG:2157). Převádíme je na standardní GPS (EPSG:4326) a
-- ukládáme do `location` (PostGIS) - to nám zatím chybělo z PPR dat a
-- je to klíčové pro budoucí zobrazení na mapě.
...
-- KROK 2b: doplnění GPS souřadnic i k nemovitostem, které už existují
-- (např. z PPR importu) a zatím žádnou polohu nemají.
UPDATE properties p
...
-- ============ KROK 3: naplnění planning_permissions ============
-- `category` = odvozený signál pro AI skórování (Retention žádost je
-- silný signál blížícího se prodeje - viz naše diskuze o "4 D").
